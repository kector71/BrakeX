Document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- INICIO: NUEVA FUNCIÓN DE AYUDA (PARA AÑOS) ---
    const parseYear = (yearStr) => {
        if (!yearStr) return null;
        const num = parseInt(yearStr, 10); // "09" -> 9, "2009" -> 2009
        if (isNaN(num)) return null;
        if (num >= 1900) return num % 100; // 2009 -> 9
        return num; // 9 -> 9
    };
    // --- FIN FUNCIÓN ---
    ...
    const getActiveFilters = () => {
        ...
        return {
            ...
            anio: normalizeText(els.anio.value), // Se normaliza aquí, se parsea en la estrategia
    ...
    // --- INICIO: REFACTORIZACIÓN (MANUFACTURER) ---
    const MANUFACTURER_TESTS = {
        'K':   (ref) => ref.startsWith('K'),
        'INC': (ref) => ref.endsWith('INC'),
        'BP':  (ref) => ref.endsWith('BP'),
        'B':   (ref) => ref.endsWith('BEX'), // El tag 'B' busca 'BEX'
    };
    // --- FIN: REFACTORIZACIÓN ---
    ...
    const FILTER_STRATEGIES = {
        ...
        // --- INICIO: FILTRO DE AÑO (CORREGIDO) ---
        anio: (item, value) => {
            // value es el input del usuario (ej: "09" o "2009", ya normalizado)
            const userInputYear = parseYear(value);
            if (userInputYear === null) return false; // Input inválido

            return (item.aplicaciones || []).some(app => {
                const dataYearStr = normalizeText(app.año); // ej: "08-14" o "07"
                if (!dataYearStr) return false;

                if (dataYearStr.includes('-')) {
                    // Es un rango: "08-14" o "2008-2014"
                    const parts = dataYearStr.split('-');
                    const startYear = parseYear(parts[0]);
                    const endYear = parseYear(parts[1]);

                    if (startYear !== null && endYear !== null) {
                        // Comprueba si el año del usuario está DENTRO del rango
                        return userInputYear >= startYear && userInputYear <= endYear;
                    }
                    return false; // Rango inválido
                } else {
                    // Es un año único: "07"
                    const singleYear = parseYear(dataYearStr);
                    if (singleYear !== null) {
                        return userInputYear === singleYear;
                    }
                    return false;
                }
            });
        },
        // --- FIN: FILTRO DE AÑO (CORREGIDO) ---
        ...
        // --- INICIO: FILTRO DE FABRICANTE (REFACTORIZADO) ---
        manufacturer: (item, manuf) => {
            // 'manuf' es la clave, ej: "K", "INC", "BP", "B"
            const testFunc = MANUFACTURER_TESTS[manuf];
            if (!testFunc) return false; // Fabricante no definido

            const allRefParts = (item.ref || []).flatMap(refStr => String(refStr).toUpperCase().split(' '));
            // Aplica la función de prueba (ej: ref.startsWith('K')) a cada parte
            return allRefParts.some(testFunc); 
        },
        // --- FIN: FILTRO DE FABRICANTE ---
